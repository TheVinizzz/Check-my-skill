...
const UserPage: FC = () => {
	...
	const [stagePage, setStagePage] = useState<IStage>({
		stage: StageView.LIST_REPOSITORIES
	} as IStage);
		...
		const listRender: any = {
			0: (
				<ListRepositories
					...
					select={(stage: IStage) => setStagePage(stage)}
				/>
			),
			1: (
				<DetailsRepository
					repository={stagePage?.nameRepository}
					select={() =>
						setStagePage({ stage: 0, nameRepository: '' })
					}
				/>
			)
		};
	...
	return (
		<>
			<Head>
				<title>{profile.login} - {profile.name}</title>
			</Head>
